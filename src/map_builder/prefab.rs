...
pub fn apply_prefab(mb: &mut MapBuilder, rng: &mut RandomNumberGenerator) {
    ...
    if let Some(placement) = placement {
        // We call chars() to convert the string to a character iterator. THen
        // we call filter on that iterator to remove carriage returns and
        // newlines. With that done, we can call collect() to change it back
        // into a string.
        let string_vec: Vec<char> = FORTRESS
            ...
            .collect();
        let mut i = 0;
        for ty in placement.y..placement.y + FORTRESS.2 - 1 {
            for tx in placement.x..placement.x + FORTRESS.1 - 1 {
